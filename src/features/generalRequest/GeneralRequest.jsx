...
import { fetchGeneralRequests } from "../../redux/slices/generalRequestSlice";
...
const GeneralRequest = () => {
  ...
  const { requests, count, loading, error } = useSelector(
    (state) => state.generalRequests
  );
  ...
  //       setAllGeneralRequest(fetchAllGeneralRequest.data);
  //     } catch (error) {
  ...
  useEffect(() => {
    dispatch(fetchGeneralRequests()); // Fetch general requests on component mount
    const interval = setInterval(() => {
      dispatch(fetchGeneralRequests()); // Fetch general requests every 2 minutes
    }, 120000);
  ...
  const filteredRequests = requests.filter((req) => {
    ...
    if (activeTab === "inprogress")
      return (
        req.InProgessStatus === true &&
        req.doorChecking === false &&
        req.CustomerFeedback === false &&
        req.CompletedStatus === false
      );
  ...
  const handleCreateRequest = async () => {
    ...
    if (response.status === 200) {
      ...
      toast.error("Request Raised failed");
    }
    setFloorName("");
    setRoomName("");
    setName("");
